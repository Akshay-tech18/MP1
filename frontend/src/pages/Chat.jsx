      ...
      <div className="flex-1 flex h-full overflow-hidden select-none">
        ...
        <div className="w-56 glass-sidebar border-r flex flex-col h-full flex-shrink-0">
          <div className="p-4 border-b dark:border-dp-dark-border-light/50 border-dp-light-border flex-shrink-0">
            <span className="font-display font-bold text-[12px] uppercase tracking-widest dark:text-dp-text-muted text-dp-text-light-muted">Messages</span>
          </div>

          <div className="flex-1 overflow-y-auto p-2.5 space-y-4">
            {/* Channels */}
            <div>
              <span className="text-[11px] font-bold dark:text-dp-text-muted text-dp-text-light-muted uppercase tracking-wider px-2.5 block mb-1.5">Channels</span>
              <button
                onClick={() => setActiveChannel({ isGroup: true, targetUser: null })}
                className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-[13px] text-left transition-all duration-150 font-semibold ${
                  activeChannel.isGroup
                    ? "dark:bg-dp-primary/10 bg-dp-primary/5 text-dp-primary"
            ...
            {/* DMs */}
            <div>
              <span className="text-[11px] font-bold dark:text-dp-text-muted text-dp-text-light-muted uppercase tracking-wider px-2.5 block mb-1.5">Direct Messages</span>
              <div className="space-y-0.5">
                {members.length === 0 ? (
                  <span className="text-[12px] dark:text-dp-text-muted text-dp-text-light-muted italic px-2.5 block py-3">No other members.</span>
                ) : (
                  members.map((member) => (
                    <button
                      key={member.userId}
                      onClick={() => setActiveChannel({ isGroup: false, targetUser: member.user })}
                      className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-[13px] text-left transition-all duration-150 font-semibold truncate ${
        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden">
          ...
          <div className="h-13 glass-sidebar border-b px-5 flex items-center flex-shrink-0">
            <div className="flex items-center gap-2.5 font-display font-bold text-[15px] dark:text-dp-text-primary text-dp-text-light-primary">
              {activeChannel.isGroup ? (
          ...
          <div ref={messagesContainerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-5 space-y-4">
            {loadingMore && (
              <div className="text-center text-[12px] dark:text-dp-text-muted text-dp-text-light-muted font-semibold py-2">Loading older messages...</div>
            )}

            {loading ? (
              <div className="flex-1 flex items-center justify-center py-20 text-sm dark:text-dp-text-muted text-dp-text-light-muted font-medium">Loading discussion...</div>
            ) : messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center select-none">
                <div className="w-14 h-14 rounded-2xl dark:bg-dp-dark-surface bg-dp-light-bg-secondary flex items-center justify-center mb-4">
                  <MessageSquare className="w-7 h-7 dark:text-dp-text-muted text-dp-text-light-muted" />
                </div>
                <h5 className="font-display font-semibold dark:text-dp-text-secondary text-dp-text-light-secondary text-sm">No Messages Yet</h5>
                <p className="text-[13px] dark:text-dp-text-muted text-dp-text-light-muted mt-1.5 max-w-xs leading-relaxed">
                  Start the discussion! Type a message below and hit send.
                </p>
              </div>
            ) : (
              messages.map((msg) => {
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[12px] dark:text-dp-text-muted text-dp-text-light-muted">{formattedTime}</span>
                      </div>
                      <p className={`mt-1.5 leading-relaxed p-3 rounded-xl inline-block max-w-lg text-[14px] border ${
                        isOwnMessage
          ...
          <div className="p-4 glass-sidebar border-t flex-shrink-0">
            <div className="h-5 mb-1.5 text-[12px] dark:text-dp-text-muted text-dp-text-light-muted italic">
              {typingText && (
            ...
            <form onSubmit={handleSendMessage} className="flex gap-3">
              ...
              />
              <button type="submit" disabled={!messageText.trim()} className="btn-primary px-5 py-2.5 flex items-center gap-2 text-[13px] disabled:opacity-40">
